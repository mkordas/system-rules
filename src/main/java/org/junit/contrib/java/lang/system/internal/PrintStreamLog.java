...
public abstract class PrintStreamLog implements TestRule {
	...
	private final ReplacePrintStream replacePrintStream;

	protected PrintStreamLog(LogMode mode, ReplacePrintStream replacePrintStream) {
		this.replacePrintStream = replacePrintStream;
		if (mode == null)
	...
	public Statement apply(final Statement base, Description description) {
		return replacePrintStream.apply(new Statement() {
			@Override
			public void evaluate() throws Throwable {
				PrintStream wrappedStream = new PrintStream(getNewStream(), NO_AUTO_FLUSH,
					ENCODING);
				replacePrintStream.replaceCurrentStreamWithStream(wrappedStream);
				base.evaluate();
			}
		}, description);
	}
	...
	private OutputStream getNewStream() throws UnsupportedEncodingException {
		switch (mode) {
			case LOG_AND_WRITE_TO_STREAM:
				return new TeeOutputStream(replacePrintStream.getStream(), log);
			case LOG_ONLY:
				return log;
			default:
